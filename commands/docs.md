...
any defaults. If it is missing, get the default via `launcher.sh templates`,
write it (and .procoder/docs/mermaid.json), then continue.

1. Run `launcher.sh docs --external` and read the whole report.
2. BLOCK lines are yours to fix before the work is finished: broken relative
   ...
   files NOT checked because a tool is missing (run `launcher.sh init`).
...
4. Re-run `launcher.sh docs --external` and show the user the report. Repeat
