...
1. Run `launcher.sh index build` if the index is missing or stale (stats
   tells you), then `launcher.sh maintain`.
2. Judge every line — nothing here blocks, everything is a candidate:
   - **unused**: dead-code candidates from the precise index. Exported API
     ...
     deletion. Confirm with `launcher.sh index refs <symbol>` before
     removing anything.
