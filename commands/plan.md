...
`launcher.sh plan list` and report.

The workflow:

1. Start from a COMPLETE spec (`launcher.sh spec check <name>` says so).
   No spec → run /procoder:spec first; the thinking happens before the
   plan, not inside it.
2. `launcher.sh plan template <name>` prints the shape; write it to the
...
4. Run `launcher.sh plan check <name>` after each pass; it names the
...
5. When COMPLETE: seed the todo list — `launcher.sh todo add` one task
