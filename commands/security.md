...
If it is missing, get the default via `launcher.sh templates`, write it, then
continue.

1. Run `launcher.sh security --deep`. Three instruments answer:
...
2. Rank what you review first with the index: `launcher.sh index entrypoints`
   is where data enters; `launcher.sh index callers <symbol>` traces how
   input reaches a finding (`launcher.sh index graph` for the full walk).
3. NOT-checked lines mean a scanner is missing — run `launcher.sh init`.
   A security check that silently didn't run is worse than a red one.
4. Fix, then re-run `launcher.sh security --deep` until no blocking findings
