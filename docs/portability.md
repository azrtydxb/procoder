...
| OpenCode                 | `opencode.json` → `.opencode/plugins/procoder.mjs`   | JS shim injects `AGENTS.md` per turn; `.opencode/command/*.md` are generated twins of `commands/*.md` (parity pinned by test). On `session.idle`/`session.compacted` the shim's `event` hook runs `procoder hook stop` — record only, a turn it cannot refuse ("What a turn end may stop"). The shim's extensions are load-bearing: both hosts discover `{plugin,plugins}/*.{ts,js}`, and this file loads only because `opencode.json` names it explicitly — the file's header records what a rename breaks |
...
### What a turn end may stop

The end of a turn is the second boundary. Claude's `Stop`/`PreCompact`
hooks and pi's `agent_settled` run `procoder hook stop` at it: the handoff
is written, and a decision that was put to the user in prose but never
recorded in `.procoder/ask/decisions.md` blocks the turn, so the recording
happens before the conversation ends. OpenCode and Kilo give a plugin no
way to refuse a turn: the shared shim's `event` hook fires on
`session.idle` and `session.compacted` and runs the same `procoder hook
stop`, but it can only record — the handoff survives to the next session
and the reminder lands in the session log, not back into the live turn.
The asymmetry is the host's: the host decides what a turn end may stop,
and this one stops nothing. The binary's verdict is identical on every
host; only the surface that carries it differs.
