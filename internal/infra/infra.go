...
// Tofu is OpenTofu, Terraform's fork, with the same fmt/validate interface.
var Tofu = &tools.Tool{
	Name:        "tofu",
...
// IaCTool picks the binary that owns a Terraform directory. A directory
// OpenTofu initialised stamps its lockfile "tofu init" and pins providers
// from registry.opentofu.org, which terraform cannot resolve, so
// `terraform validate` there always fails on providers, not on the code.
func IaCTool(dir string) *tools.Tool {
	lock, _ := os.ReadFile(filepath.Join(dir, ".terraform.lock.hcl"))
	if bytes.Contains(lock, []byte(`"tofu init"`)) || bytes.Contains(lock, []byte("registry.opentofu.org/")) {
		return Tofu
	}
	return Terraform
}
